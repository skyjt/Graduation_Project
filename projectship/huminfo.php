...
//左侧的对应栏目按钮高亮显示
$(function(){
    $("#leftmenu>ul>li>dl>dd a:eq(7)").attr("class","active");
...
<br/>
 ...
 <h1>无人船溶解氧信息查询：</h1>
  ...
  <form action="huminfo.php" method="get" id="chaxun">
...
<br/>
<p style="font-size:15px;font-weight:bold" id="hum">最高溶解氧：<span></span> &nbsp;  最低溶解氧：<span></span> &nbsp; 平均溶解氧：<span></span> </p>
<br/>
 ...
 <?php
	  ...
	  if(!empty($button)){
			...
			$rongjieyangarr=array();

			while($result=mysql_fetch_array($res)){
			echo "<tr>";
			echo "<td valign='middle'>".$result['datetime']."</td>";
                if ((double)$result['oxy']>40||(double)$result['oxy']<5){
                    echo "<td valign='middle' style='color: red'>".$result['oxy']."mg/l</td>";
                }else{
 ...
 <script>
  $(function(){
	...
	//将最高温度，最低温度信息显示出来
	$("#hum span:eq(0)").text(<?php echo max($rongjieyangarr); ?> );
	$("#hum span:eq(1)").text(<?php echo min($rongjieyangarr); ?>);
	$("#hum span:eq(2)").text(<?php echo round(array_sum($rongjieyangarr)/count($rongjieyangarr),2); ?>);
