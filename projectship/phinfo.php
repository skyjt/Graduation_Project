...
<script>
    //左侧的对应栏目按钮高亮显示
    $(function(){
        $("#leftmenu>ul>li>dl>dd a:eq(8)").attr("class","active");
...
<section class="rt_wrap content mCustomScrollbar">
    ...
    <form action="phinfo.php" method="get" id="chaxun">
    ...
    <div style="border-top:1px solid #19a97b;margin-bottom: 10%;overflow:auto">
        ...
        <br/>
        <p style="font-size:15px;font-weight:bold" id="temp">最高酸碱度：<span></span> &nbsp;  最低酸碱度：<span></span> &nbsp; 平均酸碱度：<span></span> </p>
        <br/>
            ...
            if(!empty($button)){
                ...
                if($answer == $user || $roles == '系统管理员') {
                    ...
                    while ($result = mysql_fetch_array($res)) {
                        ...
                        if ((double)$result['ph'] > 8.5 || (double)$result['ph'] < 6.5) {
        ...
        <script>
            $(function(){
                ...
                //将最高温度，最低温度信息显示出来
                $("#temp span:eq(0)").text(<?php echo max($pharr); ?> );
                $("#temp span:eq(1)").text(<?php echo min($pharr); ?>);
                $("#temp span:eq(2)").text(<?php echo round(array_sum($pharr)/count($pharr),2); ?>);
